...
ERESULT ClassMain::OnDisplayConfig(EDISPLAYCONFIG *config)
{
	...
	config->orientation     = DOR_STARTUP;        //Change orientation to match the settings of the device
...
ERESULT ClassMain::OnNextFrame(ClassEDisplay *display, unsigned long timedelta)
{
	...
	*/
	...
	display->buffer.DrawFont(2, 2, &display->fontinternal, f.GetCurrentFolderC());
	display->SetShading(E3D_GOURAUD); //E3D_TEXTURE);
			
	char name[128];
	unsigned char th, tm, ts;
	ClassEDevice::CurrentTime(th, tm, ts, true);
	display->buffer.DrawFont(2, 11, &display->fontinternal, (int)th);
	display->buffer.DrawFont(2, 20, &display->fontinternal, (int)tm);
	display->buffer.DrawFont(2, 29, &display->fontinternal, (int)ts);
	display->buffer.DrawFont(2, 38, &display->fontinternal, ecd.device->LocalTimeOffset());
	
	display->buffer.DrawFont(2, 75, &display->fontinternal, entry);
	
	display->buffer.DrawFont(2, display->GetHeight() - 2, &display->fontinternal, "Button/Stylus: Quit", EFX_COLORKEY | EFO_VBOTTOM);	
		
	ClassEStd::StrCpy(name, f.GetCurrentFolderC());
	ClassEStd::StrCat(name, "blaat.txt");
	
	//ClassEStd::StrCpy(name, "/sdcard/blaat.txt");
	
	f.New(name);
	f.Open(name, EFOF_READWRITE);
	f.Write("blaaat", 7);
	f.Close();
	f.Open(name, EFOF_READ);
	f.Read(name, 127); 
	name[127] = 0;
	f.Close();
	//ecd.device->GetOSName(name);
	display->buffer.DrawFont(2, 56, &display->fontinternal, name);	
...
void ClassMain::OnButtonUp(unsigned long bnr, EBUTTONLIST *blist)
{
	if (testdir)
	{
	    dirent * dir = readdir(testdir);
	    if (dir)
	    {
		strcpy(entry, &(dir->d_name[0]));
	    }
	    else
		strcpy(entry, "readdir fail");
	}
	else
	    strcpy(entry, "testdir fail");
}
