...
ERESULT EdgeGLInterface::Open(ENATIVETYPE wnd, unsigned long width, unsigned long height, bool stencil)
{
	...
	MacCreateOpenGLContext((void *)wnd, context, depthformat, stencilformat, true, &format, 0x100);
	if (!context)
	{
		Close();
		return(E_ERROR);
...
void EdgeGLInterface::Close(void)
{
	if (context)
	{
		MacCloseOpenGL(context, renderbuffer, framebuffer, depthbuffer, depthformat, stencilbuffer, stencilformat);
		context = 0;
	}
}
